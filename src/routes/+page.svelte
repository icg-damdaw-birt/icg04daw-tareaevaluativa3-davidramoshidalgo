<script lang="ts">
  ...
  // Marca/desmarca película como favorita a través del store
  ...
  // Califica película (1-5 estrellas) a través del store
  async function handleRate(id: string, rating: number) {
    ...
    if (ok) {
      feedbackMessage = { type: 'info', text: `Película calificada con ${rating} estrella${rating > 1 ? 's' : ''}.` };
    }
  }
